...
const EditProduct = ({ data }: { data: ProductDTO }) => {
  const { errors, handleSubmit, onSubmit, register, isPending } =
    useEditProduct(data.id!, data);
  return (
    <AlertDialog>
      <AlertDialogTrigger className="hover:bg-gray-200 cursor-pointer w-full rounded-md">
        Edit Product
      </AlertDialogTrigger>
      <AlertDialogContent>
        <AlertDialogHeader>
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction
            className="bg-yellow-500 text-white hover:bg-yellow-700 cursor-pointer rounded-md"
